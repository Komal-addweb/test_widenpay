...
class DataService
{
    ...
    Work on custome
    */
    function get_user_by_companyname($company_name){

    $user_get_companyid = db_query("SELECT n.nid,n.uid,cid.field_user_company_value from node n JOIN field_data_field_companyname c ON c.entity_id = n.nid JOIN field_data_field_user_company cid ON cid.entity_id = n.nid WHERE n.type='company_profile' AND c.field_companyname_value ='$company_name'")->fetchAll();
    
    return $user_get_companyid;
    ...
    public function Query($query, $startPosition = null, $maxResults = null)
    {
        ...
        if(empty($user->uid) || ($user->uid == 0)) {
            ...
            $company_id = isset($_GET['cid']) ? $_GET['cid'] : '';
            ...
            if(empty($company_id)) {
                ...
                $c_url = request_uri();
                if(strpos($c_url,'company') =='') {
                    
                    $baseurl = $_SERVER['HTTP_HOST'];
                    $companyname  = substr($baseurl, 0,strpos($baseurl, '.'));
                    $user_get_companyid = get_user_by_companyname($companyname);
                    $company_id = $user_get_companyid[0]->field_user_company_value;
                    ...
                    $current_url = strstr(request_uri(), "invoice/");
                    $break_url = explode('/', $current_url);
                    $company_id = $break_url[2];
                    //$userid  = get_user_by_company($company_id);
                }
            ...
            if(empty($company_id)  && isset($_GET['CUSTOMFIELD1']) && $_GET['CUSTOMFIELD1'] =='' ) {
               echo "<br/>Compay ID Missing...."; 
                die();

            }
            if(isset($_GET['CUSTOMFIELD1']) &&  $_GET['CUSTOMFIELD1'] !='')
                {
                  $company_id = $_GET['CUSTOMFIELD1'];
                }

            $result = db_query("SELECT n.nid, n.uid, cid.field_user_company_value, c.field_companyname_value from node n JOIN field_data_field_companyname c ON c.entity_id = n.nid JOIN field_data_field_user_company cid ON cid.entity_id = n.nid WHERE n.type='company_profile' AND cid.field_user_company_value ='$company_id'")->fetchAll();

            if (!empty($result)) {
                $existingUser = user_load($result[0]->uid);
                $ref_compdetail = $existingUser->field_company_ref['und'][0]['target_id'];
                $node_cmp_data = node_load($ref_compdetail);
                $user_comp = $node_cmp_data-> field_user_company['und'][0]['value'];
            }
            
        }
        else {
            $existingUser = user_load($user->uid);
            $user_comp = $existingUser->field_current_company['und'][0]['value'];
        }
       
       $this->serviceContext->realmId = trim($user_comp);
