      ...
      if (user_is_anonymous() == 0) {
        ...
        if(empty($user->uid) || ($user->uid == 0)) {
            ...
            if(strpos($c_url,'company') =='') {
                ...
                $companyname = $break_url[2];
            }

            $get_companydetail = get_user_by_companyname($companyname);
            $nid = $get_companydetail[0]->nid;
            $userid = $get_companydetail[0]->uid;
            $user_comp = $get_companydetail[0]->field_user_company_value; 
        ...
        if(isset($result['x_refresh_token_expires_in']) && isset($result['refresh_token'])) {
            ...
            $objOAuth2 = new OAuth2AccessToken($client_id, $client_secret);
            $objOAuth2->setAccessToken($result['access_token']); 
